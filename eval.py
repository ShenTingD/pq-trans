...
from models.retrieval.dump_helper_json import dump_result_json
...
def evaluate_one_epoch(test_loader, DATASET_CONFIG, CONFIG_DICT, AP_IOU_THRESHOLDS, model, criterion, config):
    ...
    for batch_idx, batch_data_label in enumerate(test_loader):
        ...
        for prefix in prefixes:
            batch_pred_map_cls, pred_mask = parse_predictions(end_points, CONFIG_DICT, prefix)  # NMS process
            ...
            if end_points['scan_name'][0] == 'scene0217_00':
                print("dumping...")
            ...
            pred_sem_cls = torch.argmax(end_points[f'{prefix}sem_cls_scores'], -1).detach().cpu().numpy()
            result = dump_result_json(end_points,  DATASET_CONFIG, pred_sem_cls)
    ...
    with open('/home/shenting/data/data225_st/pq-trans/endpoints/bbox.json', 'w') as bbox_jf:
        bbox_jf.write(results)
